const i18n = {
    en: {
        ...
        prodCmd: "Production Command", target: "Target Steel", btnMax: "⚡ Max", crafters: "Crafters", boSource: "Blood Ore Method", optAttractor: "Attractor (Fast)", optCrusher: "Crusher (Byproducts)",
        ...
        items: { granum: "Granum", calx: "Calx", saburra: "Saburra", water: "Water", sp: "Saburra Powder", cp: "Calx Powder", coal: "Coal", coke: "Coke", pi: "Pig Iron", gs: "Grain Steel", steel: "Steel", bo: "Blood Ore", amarantum: "Amarantum", flakestone: "Flakestone", granumpowder: "Granum Powder", malachite: "Malachite", bleckblende: "Bleckblende", jadeite: "Jadeite", calspar: "Calspar" },
        stepGrind: "Grind", stepCrush: "Crush", stepAttract: "Attract", stepFurnace: "Furnace", stepOven: "Oven", stepFinal: "Final Oven:", stepTo: "to", stepYields: "yields", stepPlus: "+", perCrafter: "(Per Crafter)",
...
function changeLang() {
    ...
    ['interface', 'themeToggle', 'format', 'optUnits', 'optStacks', 'webhook', 'prodCmd', 'target', 'btnMax', 'crafters', 'boSource', 'optAttractor', 'optCrusher', 'yieldMods', 'mastery', 'refining', 'extraction', 'logStats', 'totalStacksLabel', 'totalGoldLabel', 'btnDiscord', 'btnSend', 'invBank', 'btnReset', 'defGather', 'mfgPipe', 'marketCart', 'btnAutoFill'].forEach(id => {
...
function runEngine(targetRaw, bankData, mode) {
    ...
    const boSource = document.getElementById('boSource').value;
    ...
    let bp = { coal: 0, amarantum: 0, flakestone: 0, granumpowder: 0, malachite: 0, bleckblende: 0, jadeite: 0, calspar: 0, cp: 0 };
    ...
    if (boSource === 'crusher') {
        ...
        bp.flakestone += Math.ceil(rGranum * 0.0140 * mE);
        bp.granumpowder += Math.ceil(rGranum * 0.2940 * mE);
    } else {
        rGranum = Math.ceil(rBlood / (0.198 * mE * mM));
        cpForBlood = Math.ceil(rGranum * 0.0715);
    }
    ...
    bp.cp += Math.ceil(calxToCrush * 0.1361 * mE);
    bp.flakestone += Math.ceil(calxToCrush * 0.0180 * mE);
    bp.malachite += Math.ceil(calxToCrush * 0.0891 * mE);
...
function calculate() {
    ...
    if (res.rGranum > 0) {
        if (res.boSource === 'crusher') {
            ...
            steps.push(`${t.stepAttract}: <span class="highlight">${s(res.rGranum)} ${t.items.granum}</span> ${t.stepPlus} <span class="highlight">${s(res.cpForBlood)} ${t.items.cp}</span> ${t.stepYields} <span class="highlight">${s(res.rBlood)} ${t.items.bo}</span>`);
        }
    }
    ...
    Object.keys(res.byproducts).forEach(k => {
        if (res.byproducts[k] > 0) {
            byproductsString += `<div style="display:flex; justify-content:space-between; margin-bottom: 2px;">
                ...
                <span style="color: var(--accent); font-weight: bold;">${res.byproducts[k].toLocaleString()}</span>
            </div>`;
        }
    });
